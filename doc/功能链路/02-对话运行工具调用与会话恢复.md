...
代码仓库类请求在首次模型调用前先经过本地 `NavigationResolver`。它使用功能目录和工作区索引
生成有限的候选文件、符号与推荐只读动作；高置信度直接定位和低置信度搜索使用
`tool_choice=required`，取得首个工具结果后恢复 `auto`；失败结果也会交还模型解释或修正，避免
权限拒绝等确定性结果陷入重复强制调用。工具发现阶段不请求最终摘要 Schema，避免
Provider 同时被要求调用工具和生成最终 JSON。设置 `AGENT_NAVIGATION_MODE=off` 可关闭该链路。
...
CLI 和 TUI 共用命令解析契约。输入 `/` 后候选按当前界面、运行状态和可用依赖过滤；方向键会滚动可视窗口，`Tab` 只补全，`Enter` 执行完整的无参数命令或确认已补全的参数命令。路径、任务 ID、会话 ID 和检查点由独立的本地元数据补全器提供，异步结果会校验输入版本，避免旧结果覆盖新输入。

| 命令 | 数据流和副作用 |
| --- | --- |
| `/sessions` | 读取当前工作区 `.echolens/sessions/*.jsonl` 的元数据；不读取完整对话正文，不删除任何数据。 |
| `/session delete <session-id>` | 再次读取列表并确认目标的大小/修改时间；拒绝当前 Session，获取目标 `.jsonl.lock` 后只删除该日志；检查点、产物、代码和其他会话保留。 |
| `/clear` | 仅 TUI 清空内存 transcript，不删除 Session JSONL，也不改变模型历史。 |
| `/steer <要求>` | 运行中写入 `turn.steered` 并排队由下一模型步骤消费；暂停后写入并继续 `/resume`，已完成 Turn 不接受 steering。 |

删除操作使用 Node `unlink` 的文件级语义，目录必须是规范真实目录、目标必须是普通文件；确认期间文件发生变化、被其他写者锁定或路径为符号链接时失败关闭。
...
- `/cd <path>` 或 `/workspace <path>`：关闭当前 Session 并在目标目录创建新 Session，不跨目录复制历史或检查点。
...
`runtime.test.ts`、`tool-scheduler.test.ts`、`tool-errors.test.ts`、`guardrail.test.ts`、`jsonl-event-store.test.ts`、`session-runtime.test.ts`、`workspace-manager.test.ts`、`tui.test.ts`。
