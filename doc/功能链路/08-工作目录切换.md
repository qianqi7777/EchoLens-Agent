...
候选按当前上下文裁剪：运行中的 Turn 仅保留 `/steer`；没有工作区或后台任务服务时，对应命令不会显示。参数命令会异步提供路径、任务 ID、会话 ID 和检查点候选，旧输入的异步结果会被丢弃。第一版内置命令统一注册，后续可在同一目录协议上接入 Skill、插件和 MCP 命令。
...
- `/sessions`、`/session delete <session-id>`、`/verify`、`/rollback`、`/task`、`/resume` 和普通提问始终转发到当前工作区运行时。删除历史会话需要二次确认，当前 Session 永远拒绝删除。
